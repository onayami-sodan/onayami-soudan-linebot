...
export async function handleLove(event) {
  ...
  // flowガード：トリガー or 進行中 以外は黙殺
  if (s0?.flow !== 'love40' && !trigger) return

  // duplicate guard（このフローを扱うと決めてから）
  ...
  if (s?.love_step === 'PRICE') {
    ...
    if (GENDER_OPTIONS.includes(tn)) {
      ...
      await sendAgeFlex(event)
      return
    }

    // ★寛容：PRICEで年代が来たら性別不明でQへ
  ...
  if (s?.love_step === 'PROFILE_GENDER') {
    if (!GENDER_OPTIONS.includes(tn)) {
      await safeReply(event.replyToken, { type: 'flex', altText: '性別を選んでね',
  ...
  // ===== Q（id一致＆未回答のみ） =====
  if (s?.love_step === 'Q') {
    // 誤タップ耐性：Q中に性別/年代が来ても無視して現行Qを再掲
    if (GENDER_OPTIONS.includes(tn) || AGE_OPTIONS.includes(tn)) {
...
async function sendNextLoveQuestion(event, session) {
  ...
  if (idx >= (QUESTIONS?.length || 0)) {
    ...
    await push(event.source?.userId, buildFinalConfirmFlex())
    return true
  }
